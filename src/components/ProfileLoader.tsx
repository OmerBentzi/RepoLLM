...
export function ProfileLoader({ username }: ProfileLoaderProps) {
    ...
    const loadProfile = async () => {
        try {
            ...
            const profile: GitHubProfile | null = await fetchProfile(username) as GitHubProfile | null;
            ...
            updateStep("repos", "loading", `Found ${profile.public_repos || 0} repositories`);
            let repoReadmes: any[] = [];
            try {
                repoReadmes = await fetchUserRepos(username) || [];
            } catch (e) {
                // Profile mode not supported in local-only mode
                updateStep("repos", "complete", "Profile mode not supported");
                repoReadmes = [];
            }
            
            if (repoReadmes.length > 0) {
